...
-- Параметры заполнены на отчётную дату 31.12.2025
-- ТРЕБУЕТ СВЕРКИ с методруководством НСТ-2026, когда оно получено.

-- Параметры на 31.12.2025
-- Источники:
-- @capital_nst2026: Финансовая отчётность на 31.12.2025 (регуляторный капитал)
-- @mrp_2025: Закон РК «О республиканском бюджете на 2025–2027 годы» (МРП = 3,932 тенге)
DECLARE @capital_nst2026 FLOAT = 557685150000;  -- тыс.тенге * 1000 (регуляторный капитал на 31.12.2025)
DECLARE @mrp_2025 FLOAT = 3932;                 -- МРП на 01.01.2025 по закону о бюджете
DECLARE @threshold_individual FLOAT = 0.002;    -- 0,2% по Таблице 4, п. Individual loans
DECLARE @threshold_ead_retail FLOAT = 200000000;-- 200M threshold для розницы

-- Граница крупного бизнеса: > 3,000,000 МРП (Entrepreneurship Code § 6)
-- Значение: 3,000,000 * 3,932 = 11,796,000,000 тенге
DECLARE @large_biz_income FLOAT = @mrp_2025 * 3000000;

-- Граница малого бизнеса: <= 300,000 МРП (§ 3)
-- Значение: 300,000 * 3,932 = 1,179,600,000 тенге
DECLARE @small_biz_income FLOAT = @mrp_2025 * 300000;

-- Диагностика параметров (для логов):
-- Individual loans threshold: 0.2% of @capital_nst2026 = 1,153,370,300 тенге
-- CORLAR (large): > @large_biz_income = 11,796,000,000 тенге
-- RETSML (small): <= @small_biz_income = 1,179,600,000 тенге
-- CORMED: между RETSML и CORLAR
