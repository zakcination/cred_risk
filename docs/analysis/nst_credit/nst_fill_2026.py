...
def fill(args):
    ...
    if args.write == "sidecar":
        ...
        # Не известен ни оттуда, ни оттуда — отказываем, а не «пробуем».
        #
        # Замер: 200 тыс. строк × 115 колонок inplace отрабатывает, так что
        # порог здесь не про крах, а про время и память — на 566 тысячах
        # запас неизвестен и проверять его на рабочей машине незачем,
        # когда есть stream. --force снимает отказ, если это осознанно.
        big, unknown = [], []
