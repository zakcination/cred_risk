/* =============================================================================
   ...
   Зерно: месяц × источник. Окно: 24 полных месяца до даты снимка loans —
...
, iss AS (          /* 2-1: выдачи по месяцу l_funding_date */
    ...
    GROUP BY DATEFROMPARTS(YEAR(b.l_funding_date), MONTH(b.l_funding_date), 1), b.l_source
)
, cl AS (           /* 2-3: закрытия по месяцу l_actual_closure_date */
    ...
    GROUP BY DATEFROMPARTS(YEAR(b.l_actual_closure_date), MONTH(b.l_actual_closure_date), 1), b.l_source
)
, k AS (
    SELECT month_start, l_source FROM iss
    UNION
    SELECT month_start, l_source FROM cl
)
...
FROM      k
LEFT JOIN iss AS i ON i.month_start = k.month_start AND i.l_source = k.l_source
LEFT JOIN cl  AS c ON c.month_start = k.month_start AND c.l_source = k.l_source
OPTION (MAXDOP 1);
