...
def sheet_howto(wb):
    ...
    text = [
        ...
        "",
        "ОГОВОРКА, КОТОРУЮ НАДО ЗНАТЬ. Две метрики CoR размечены квантилем изменений "
        "по 29 обычным месяцам (без февраля–марта) и предварительно: формула расчёта "
        "ряда не подтверждена. На листе «Пороги» они помечены.",
    ]
    r = 4
    for t in text:
...
def sheet_thresholds(wb):
    ...
    for r in res:
        if r["method"] == "M(T)":
            ...
            note = ("квантиль изменений по 29 обычным месяцам (без фев–мар). "
                    "ПРЕДВАРИТЕЛЬНО — формула расчёта CoR не подтверждена")
...
def selftest():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        # Дополнение к записке размечает CoR квантилем по 29 обычным месяцам.
        # Граница −0,29 означала бы, что лист снова считает CoR формулой по всем
        # 35 изменениям и расходится с дополнением.
        ...
        check("границы зелёной по CoR — замещающим методом, совпадают с запиской",
              [round(g, 2) for g in cor_green] == [1.57, 2.49], str(cor_green))
    return ok
