...
Редакция 15.09.2026, седьмая:
  • лист «Нормальность» — шаг 1: подчиняются ли приращения нормальному
    распределению. Моменты и Жарк-Бера считаются живыми формулами через
    SUMPRODUCT, а НЕ через СКОС и ЭКСЦЕСС: встроенные функции возвращают
    исправленные на смещение оценки, а критерий построен на выборочных
    моментах — подстановка дала бы неверную статистику при верной на вид
    формуле. Шапиро-Уилк и Андерсон-Дарлинг в Excel без макросов не
    воспроизводятся и внесены константами с пометкой — второе объявленное
    исключение после бутстрапа. Нормальны ТРИ метрики из двенадцати.
  • лист «CoR — зоны» — шаг 3: действующая разметка обеих CoR режимной
    исторической симуляцией. Эмпирический 90-й процентиль T-месячных
    изменений считается раздельно для февраля-марта и остальных месяцев
    (режимы Г36), каждая точка оценивается границей своего месяца.
    Раскладка: КБ/ПБ 34 зелёных и 2 жёлтых, МСБ 36 зелёных; текущая точка
    у обеих ЗЕЛЁНАЯ.
  • зона CoR на листе «Метрики» объявлена НЕДЕЙСТВИТЕЛЬНОЙ и оставлена
    намеренно: по ней видно, что даёт M(T) на этой метрике и почему метод
    пришлось заменить. Прочие одиннадцать метрик остаются на M(T) — замена
    понадобилась одной из двенадцати, а не девяти.
...
OUT = os.path.join(HERE, "..", "out", "Raschetny_list_RA_top20_v1.7.xlsx")
...
def sheet_metrics(wb, calc, rb_first):
    ...
    for title, text in (
        ...
        ("У CoR КБ/ПБ буфер больше самого лимита — и причина не в зонах (Г36)",
         ...
         "Смешанная σ прячет обе половины. ЗОНА CoR В ЭТОЙ СТРОКЕ НЕДЕЙСТВИТЕЛЬНА — "
         "действующая разметка на листе «CoR — зоны», где применена режимная "
         "историческая симуляция. Строка оставлена намеренно: по ней видно, что́ даёт "
         "M(T) на этой метрике и почему метод пришлось заменить. Светофор здесь "
         "переключается: 25 / 9 / 2 / 0; на действующей разметке 34 / 2 / 0 / 0."),
...
# ── листы «Нормальность» и «CoR — зоны» ───────────────────────────────────
# Опубликованные значения — из calib/normality_triage.py. Здесь они только
# сверяются: сами числа лист считает формулами.
NORM_EXPECTED = {
    "top20": (0.673820, 3.189553, 2.700926, 0.259120, 0.109985, 0.620456),
    "cor_kb_pb": (-2.117464, 12.498891, 157.738495, 0.000000, 0.000004, 2.383589),
    "cor_msb": (-0.917560, 8.047658, 42.067840, 0.000000, 0.000208, 1.766130),
    "el_total": (-1.569662, 7.733692, 47.050498, 0.000000, 0.001168, 1.075610),
    "el_unsecured": (0.180676, 4.457584, 3.288724, 0.193136, 0.062194, 0.955278),
    "el_secured": (-0.164822, 2.935362, 0.164563, 0.921013, 0.748535, 0.277056),
    "el_other": (-0.018164, 3.490380, 0.352614, 0.838361, 0.695118, 0.321707),
    "pd_unsecured": (-0.064308, 5.814395, 11.575316, 0.003065, 0.010480, 1.215961),
    "pd_unsecured_cash": (-0.185566, 6.249631, 15.601022, 0.000410, 0.003629, 1.403588),
    "pd_unsecured_goods": (-1.090441, 3.936156, 8.214265, 0.016455, 0.005434, 1.182294),
    "pd_secured": (0.148249, 5.803641, 11.591291, 0.003041, 0.001876, 1.615887),
    "pd_other": (0.287274, 4.783112, 5.118155, 0.077376, 0.064780, 0.775723),
}

# (q1_обычн, q4_обычн_сырой, q1_закр, q4_закр_сырой,
#  гр.зел_обычн, гр.жёлт_обычн, гр.зел_закр, гр.жёлт_закр, зел, жёлт, красн, наруш)
COR_EXPECTED = {
    "cor_kb_pb": (0.718255, 1.427255, 2.269362, 1.756548,
                  1.572745, 2.281745, 0.730638, 0.730638, 34, 2, 0, 0),
    "cor_msb": (0.374285, 1.014138, 1.538174, 1.422467,
                2.485862, 3.125715, 1.961826, 1.961826, 36, 0, 0, 0),
}


def sheet_normality(wb, rb_first):
    """Шаг 1: какие метрики подчиняются нормальному распределению.

    Моменты считаются напрямую через SUMPRODUCT, а не через СКОС и ЭКСЦЕСС:
    встроенные функции Excel возвращают исправленные на смещение оценки,
    а критерий Жарк-Бера построен на выборочных моментах. Подставить одно
    вместо другого — получить неверную статистику при верно выглядящей формуле.

    Шапиро-Уилк и Андерсон-Дарлинг в Excel без макросов не воспроизводятся
    (первому нужна таблица коэффициентов, второму — сортировка ряда). Они
    приведены КОНСТАНТАМИ и помечены — это второе такое исключение в книге
    после бутстрапа, и оба объявлены, а не спрятаны.
    """
    ws = wb.create_sheet("Нормальность")
    put(ws, "A1", "Шаг 1: подчиняются ли приращения нормальному распределению",
        F_TITLE)
    note(ws, 2, "O",
         "Проверяются приращения Δ каждой метрики, n = 35. Жарк-Бера считается "
         "живой формулой из моментов ряда; Шапиро-Уилк и Андерсон-Дарлинг в Excel "
         "без макросов не считаются и внесены константами — они помечены. Важно: "
         "эта проверка НЕ решающая. M(T) опирается не на приращение, а на сумму T "
         "приращений, которая по ЦПТ ближе к нормальной. Решает лист «Метрики» — "
         "там модельный квантиль сравнивается с фактическим.", 58)

    cols = [("A", "Метрика", 30), ("B", "Ряд приращений", 22), ("C", "n", 6),
            ("D", "Среднее", 10), ("E", "m₂", 11), ("F", "m₃", 12), ("G", "m₄", 12),
            ("H", "Асимметрия S", 12), ("I", "Эксцесс K", 10),
            ("J", "Жарк-Бера", 11), ("K", "p(ЖБ)", 10), ("L", "Вердикт по ЖБ", 13),
            ("M", "Шапиро p ⚠", 11), ("N", "Андерсон A² ⚠", 12), ("O", "Итог", 12)]
    for col, t, w in cols:
        put(ws, f"{col}4", t, F_HEAD, fill=HEAD_BG, border=True,
            align=Alignment(wrap_text=True, vertical="bottom"))
        ws.column_dimensions[col].width = w
    ...
    ranges = [("top20", "Топ-20 / СК", "'Ряд'!J6:J40"),
              ("cor_kb_pb", "CoR КБ/ПБ", "'Ряд'!P6:P40"),
              ("cor_msb", "CoR МСБ", "'Ряд'!S6:S40")]
    for i, (code, _n, _s, _l, _v) in enumerate(METRICS_RB):
        r = rb_first + i
        ranges.append((code, TITLES[code].replace("ожидаемые потери, ", ""),
                       f"'Метрики — ряд'!{get_column_letter(41)}{r}:"
                       f"{get_column_letter(75)}{r}"))

    checks, r = [], 5
    for code, name, rng in ranges:
        s, k, jb, p, sw, ad = NORM_EXPECTED[code]
        put(ws, f"A{r}", name, F_BODY, align=WRAP, border=True)
        put(ws, f"B{r}", rng.replace("'", ""), F_MONO, align=WRAP, border=True)
        put(ws, f"C{r}", f"=COUNT({rng})", F_BODY, "0", border=True, align=CENTER)
        put(ws, f"D{r}", f"=AVERAGE({rng})", F_BODY, "0.0000", border=True)
        for col, pw in (("E", 2), ("F", 3), ("G", 4)):
            put(ws, f"{col}{r}", f"=SUMPRODUCT(({rng}-D{r})^{pw})/C{r}",
                F_BODY, "0.000000", border=True)
        put(ws, f"H{r}", f"=F{r}/E{r}^1.5", F_BLOCK, "0.0000", border=True)
        put(ws, f"I{r}", f"=G{r}/E{r}^2", F_BLOCK, "0.0000", border=True)
        put(ws, f"J{r}", f"=C{r}/6*(H{r}^2+(I{r}-3)^2/4)", F_BLOCK, "0.0000",
            border=True)
        put(ws, f"K{r}", f"=CHIDIST(J{r},2)", F_BLOCK, "0.0000", border=True)
        put(ws, f"L{r}", f'=IF(K{r}>0.05,"норма","НЕ норма")', F_BODY,
            border=True, align=CENTER, fill=GREEN_BG if p > 0.05 else RED_BG)
        put(ws, f"M{r}", sw, F_MUTED, "0.0000", border=True)
        put(ws, f"N{r}", ad, F_MUTED, "0.0000", border=True)
        put(ws, f"O{r}", f'=IF(AND(K{r}>0.05,M{r}>0.05,N{r}<0.752),'
                         f'"НОРМА","НЕ норма")', F_BLOCK, border=True, align=CENTER,
            fill=GREEN_BG if (p > 0.05 and sw > 0.05 and ad < 0.752) else RED_BG)
        ws.row_dimensions[r].height = 26
        for col, pub, tol in (("H", s, 0.00005), ("I", k, 0.00005),
                              ("J", jb, 0.00005), ("K", p, 0.00005)):
            checks.append(("НОРМАЛЬНОСТЬ", f"{col}{r}", pub, tol))
        r += 1

    r += 1
    for title, text in (
        ("Нормальны три метрики из двенадцати",
         "По всем трём критериям проходят топ-20 (эксцесс 3,19), EL обеспеченные "
         "(2,94) и EL прочие (3,49). По одному Жарк-Бера проходят пять — он мягче "
         "Шапиро-Уилка на таком n. Хуже всех CoR КБ/ПБ: асимметрия −2,12, "
         "эксцесс 12,50, Жарк-Бера 157,7 против критических 5,99."),
        ("Но девять «ненормальных» валятся не из-за хвостов, а из-за сноса",
         "EL и PD падают три года подряд, а M(T) считает снос нулевым. Поэтому "
         "модель завышает буфер — по PD товарным в 6 раз. Завышение безопасно, "
         "и снос в зачёт сознательно не берётся: падение PD не обещает продолжения. "
         "Замена метода нужна ОДНОЙ метрике из двенадцати — CoR КБ/ПБ, где "
         "M(4) = 3,2850 превышает сам лимит 3,00 и зона вырождается."),
        ("Почему эта таблица не решает",
         "Ненормальность приращения не запрещает M(T): формуле нужен квантиль "
         "СУММЫ T приращений, а сумма по ЦПТ ближе к нормальной, чем слагаемое. "
         "На топ-20 это измерено: асимметрия приращения +0,6738 против +0,0448 "
         "у трёхмесячной суммы. Решающая проверка — на листе «Метрики»."),
    ):
        put(ws, f"A{r}", title, F_BLOCK, align=WRAP, fill=BLOCK_BG, border=True)
        put(ws, f"B{r}", text, F_BODY, align=WRAP, border=True)
        ws.merge_cells(f"B{r}:O{r}")
        ws.row_dimensions[r].height = max(40, 13 * (len(text) // 130 + 1))
        r += 1
    ws.freeze_panes = "C5"
    return checks


def sheet_cor_zones(wb, rows):
    """Шаг 3: зоны CoR режимной исторической симуляцией (Г36).

    Единая σ размазывает февральский всплеск по всему году; эмпирический
    квантиль, посчитанный ОТДЕЛЬНО для окна закрытия и для обычных месяцев,
    его удерживает. Каждая точка ряда оценивается границей своего месяца.
    """
    ws = wb.create_sheet("CoR — зоны")
    put(ws, "A1", "Шаг 3: зоны CoR — режимная историческая симуляция", F_TITLE)
    note(ws, 2, "U",
         "CoR КБ/ПБ единственная из двенадцати метрик, где M(T) непригодна: "
         "M(4) = 3,2850 больше собственного лимита 3,00, граница зелёной уходит "
         "в минус. Замена — эмпирический 90-й процентиль фактических T-месячных "
         "изменений (BCBS d457 MAR30 допускает историческую симуляцию наравне "
         "с параметрической моделью), посчитанный раздельно по режимам Г36: "
         "февраль-март против остальных месяцев. CoR МСБ размечен тем же методом "
         "для сопоставимости. ВАЖНО: открыта Д15 — чем является ряд CoR, "
         "неизвестно, и любая его разметка предварительна.", 62)

    ph = [("A", "Метрика", 16), ("B", "Q90 1м обычн", 12),
          ("C", "Q90 4м обычн (сырой)", 14), ("D", "Q90 4м обычн (выправл.)", 15),
          ("E", "Q90 1м закр", 12), ("F", "Q90 4м закр (сырой)", 14),
          ("G", "Q90 4м закр (выправл.)", 15), ("H", "Лимит", 9),
          ("I", "Зел./жёлт. обычн", 14), ("J", "Жёлт./красн. обычн", 15),
          ("K", "Зел./жёлт. закр", 14), ("L", "Жёлт./красн. закр", 15),
          ("M", "Зелёных", 9), ("N", "Жёлтых", 9), ("O", "Красных", 9),
          ("P", "Нарушений", 10)]
    for col, t, w in ph:
        put(ws, f"{col}4", t, F_HEAD, fill=HEAD_BG, border=True,
            align=Alignment(wrap_text=True, vertical="bottom"))
    ...
    D0, D1 = 12, 12 + N_POINTS - 1            # строки данных
    checks = []
    SPEC = [("cor_kb_pb", "CoR КБ/ПБ", 3.0, 5, "F", "G", "J", "K", "L"),
            ("cor_msb", "CoR МСБ", 3.5, 6, "O", "P", "S", "T", "U")]
    for code, name, lim, pr, c1o, c1c, c4o, c4c, zcol in SPEC:
        e = COR_EXPECTED[code]
        put(ws, f"A{pr}", name, F_BODY, border=True)
        put(ws, f"B{pr}", f"=PERCENTILE({c1o}{D0}:{c1o}{D1},p_conf)", F_BLOCK,
            "0.0000", border=True)
        put(ws, f"C{pr}", f"=PERCENTILE({c4o}{D0}:{c4o}{D1},p_conf)", F_BODY,
            "0.0000", border=True)
        put(ws, f"D{pr}", f"=MAX(C{pr},B{pr})", F_BLOCK, "0.0000", border=True)
        put(ws, f"E{pr}", f"=PERCENTILE({c1c}{D0}:{c1c}{D1},p_conf)", F_BLOCK,
            "0.0000", border=True)
        put(ws, f"F{pr}", f"=PERCENTILE({c4c}{D0}:{c4c}{D1},p_conf)", F_BODY,
            "0.0000", border=True)
        put(ws, f"G{pr}", f"=MAX(F{pr},E{pr})", F_BLOCK, "0.0000", border=True)
        put(ws, f"H{pr}", lim, F_BODY, "0.00", border=True)
        put(ws, f"I{pr}", f"=H{pr}-D{pr}", F_BLOCK, "0.0000", border=True,
            fill=GREEN_BG)
        put(ws, f"J{pr}", f"=H{pr}-B{pr}", F_BLOCK, "0.0000", border=True,
            fill=YELLOW_BG)
        put(ws, f"K{pr}", f"=H{pr}-G{pr}", F_BLOCK, "0.0000", border=True,
            fill=GREEN_BG)
        put(ws, f"L{pr}", f"=H{pr}-E{pr}", F_BLOCK, "0.0000", border=True,
            fill=YELLOW_BG)
        for col, word, bg in (("M", "зелёная", GREEN_BG), ("N", "жёлтая", YELLOW_BG),
                              ("O", "красная", RED_BG), ("P", "нарушение", WARN_BG)):
            put(ws, f"{col}{pr}", f'=COUNTIF({zcol}{D0}:{zcol}{D1},"{word}")',
                F_BLOCK, "0", border=True, align=CENTER, fill=bg)
        for col, pub, tol in (("B", e[0], 0.00005), ("C", e[1], 0.00005),
                              ("E", e[2], 0.00005), ("F", e[3], 0.00005),
                              ("I", e[4], 0.00005), ("J", e[5], 0.00005),
                              ("K", e[6], 0.00005), ("L", e[7], 0.00005),
                              ("M", e[8], 0.5), ("N", e[9], 0.5),
                              ("O", e[10], 0.5), ("P", e[11], 0.5)):
            checks.append(("COR — ЗОНЫ", f"{col}{pr}", pub, tol))
        ws.row_dimensions[pr].height = 24

    dh = [("A", "Дата", 12), ("B", "Мес", 6), ("C", "Закрытие?", 9),
          ("D", "CoR КБ/ПБ", 11), ("E", "Δ", 10), ("F", "Δ обычн", 10),
          ("G", "Δ закр", 10), ("H", "Изм. 4 мес", 11), ("I", "Окно закр?", 10),
          ("J", "4м обычн", 10), ("K", "4м закр", 10), ("L", "Зона КБ/ПБ", 12),
          ("M", "CoR МСБ", 11), ("N", "Δ", 10), ("O", "Δ обычн", 10),
          ("P", "Δ закр", 10), ("Q", "Изм. 4 мес", 11), ("R", "Окно закр?", 10),
          ("S", "4м обычн", 10), ("T", "4м закр", 10), ("U", "Зона МСБ", 12)]
    for col, t, w in dh:
        put(ws, f"{col}{D0 - 1}", t, F_HEAD, fill=HEAD_BG, border=True,
            align=Alignment(wrap_text=True, vertical="bottom"))
        ws.column_dimensions[col].width = w
    ws.row_dimensions[D0 - 1].height = 32

    for i in range(N_POINTS):
        r, sr = D0 + i, FIRST + i
        put(ws, f"A{r}", f"='Ряд'!A{sr}", F_BODY, border=True)
        put(ws, f"B{r}", f"=MID(A{r},6,2)", F_MUTED, border=True, align=CENTER)
        put(ws, f"C{r}", f'=IF(OR(B{r}="02",B{r}="03"),1,0)', F_MUTED, "0",
            border=True, align=CENTER)
        for val_c, src, d_c, do_c, dc_c, f4_c, fl_c, o4_c, c4_c, z_c, pr in (
                ("D", "O", "E", "F", "G", "H", "I", "J", "K", "L", 5),
                ("M", "R", "N", "O", "P", "Q", "R", "S", "T", "U", 6)):
            put(ws, f"{val_c}{r}", f"='Ряд'!{src}{sr}", F_BODY, "0.0000", border=True)
            if i == 0:
                for c in (d_c, do_c, dc_c):
                    put(ws, f"{c}{r}", "—", F_MUTED, border=True, align=CENTER)
            else:
                put(ws, f"{d_c}{r}", f"={val_c}{r}-{val_c}{r - 1}", F_BODY,
                    "0.0000", border=True)
                put(ws, f"{do_c}{r}", f'=IF(C{r}=0,{d_c}{r},"")', F_BODY,
                    "0.0000", border=True)
                put(ws, f"{dc_c}{r}", f'=IF(C{r}=1,{d_c}{r},"")', F_BODY,
                    "0.0000", border=True)
            if i < 4:
                for c in (f4_c, fl_c, o4_c, c4_c):
                    put(ws, f"{c}{r}", "—", F_MUTED, border=True, align=CENTER)
            else:
                put(ws, f"{f4_c}{r}", f"={val_c}{r}-{val_c}{r - 4}", F_BODY,
                    "0.0000", border=True)
                put(ws, f"{fl_c}{r}", f"=IF(SUM(C{r - 3}:C{r})>0,1,0)", F_MUTED,
                    "0", border=True, align=CENTER)
                put(ws, f"{o4_c}{r}", f'=IF({fl_c}{r}=0,{f4_c}{r},"")', F_BODY,
                    "0.0000", border=True)
                put(ws, f"{c4_c}{r}", f'=IF({fl_c}{r}=1,{f4_c}{r},"")', F_BODY,
                    "0.0000", border=True)
            put(ws, f"{z_c}{r}",
                f'=IF({val_c}{r}<IF(C{r}=1,$K${pr},$I${pr}),"зелёная",'
                f'IF({val_c}{r}<IF(C{r}=1,$L${pr},$J${pr}),"жёлтая",'
                f'IF({val_c}{r}<$H${pr},"красная","нарушение")))',
                F_BODY, border=True, align=CENTER)

    r = D1 + 2
    for title, text in (
        ("Раскладка 36 точек: КБ/ПБ — 34 зелёных, 2 жёлтых; МСБ — 36 зелёных",
         "Текущая точка 01.07.2026 у обеих ЗЕЛЁНАЯ. На M(T) у КБ/ПБ была ЖЁЛТАЯ, "
         "и это расхождение — не уточнение, а смена метода: там буфер 3,2850 "
         "превышал лимит 3,00, здесь он равен 1,4273 по обычным месяцам."),
        ("В окне закрытия лестница вырождается в две зоны",
         "Q90 четырёхмесячных изменений там МЕНЬШЕ месячного (1,7565 против 2,2694 "
         "по КБ/ПБ), потому что снос съедает рост на длинном окне. Колонка D берёт "
         "MAX и выправляет порядок, из-за чего границы зелёной и жёлтой совпадают: "
         "в феврале-марте зон две, а не три. Это свойство метода, а не округление."),
        ("Цена метода: в окне закрытия шесть наблюдений",
         "Три года по два месяца. Квантиль 0,90 по шести наблюдениям есть "
         "фактически максимум выборки. Повысить точность нечем, кроме удлинения "
         "ряда. Для обычных месяцев наблюдений 29 и 17 — тоже немного."),
        ("Открыта Д15, и она делает всю разметку предварительной",
         "Строки CoR на листе Monthly вставлены без указания источника (08.09.2026, "
         "при чтении формул: EL и PD там внешние ссылки, CoR — просто числа). "
         "Месячный расход, YTD или LTM — неизвестно. Один вопрос владельцу файла "
         "закрывает и Д15, и объяснение Г36."),
    ):
        put(ws, f"A{r}", title, F_BLOCK, align=WRAP, fill=BLOCK_BG, border=True)
        put(ws, f"B{r}", text, F_BODY, align=WRAP, border=True)
        ws.merge_cells(f"B{r}:U{r}")
        ws.row_dimensions[r].height = max(40, 13 * (len(text) // 150 + 1))
        r += 1
    ws.freeze_panes = "D12"
    return checks
...
def build():
    ...
    extra += sheet_normality(wb, rb_first)
    extra += sheet_cor_zones(wb, rows)
    sheet_guide(wb)
