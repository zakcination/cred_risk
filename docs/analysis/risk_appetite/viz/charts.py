...
def ex10():
    ...
    ax.text(100, -1.10, "Нефть заявлена корнем причинной цепочки сценария, но входит в формулу\n"
                        "параллельным слагаемым с весом 0,11 % портфеля: каскада «нефть → курс →\n"
                        "залоги» в расчёте нет, факторы задаются независимо друг от друга.",
            ha="right", va="center", fontsize=10.5, color=INK, weight="bold")
    frame(fig, "Причинная цепочка заявлена в описании, но не заложена в модель",
