/* =============================================================================
   ...
   ПОПРАВКА 27.08.2026, ЧИТАТЬ ДО ЗАПУСКА. См. METHOD.md, П3 и П5.

   Снимочный DPD почти детерминированно равен (дней в предыдущем месяце −
   день платежа) + 1. Значит его УРОВЕНЬ между заёмщиками несравним, и
   «стабильные 6 дней» не означают «опаздывает на 6 дней»: тот же заёмщик
   с днём платежа 25-го и реальной просрочкой 20 дней покажет на срезе 6.

   Что разброс всё-таки различает — тип просрочки, а не её размер:
     низкий разброс  -> ПЕРЕКАТ: платит каждый месяц, но с опозданием
     высокий разброс -> НАКОПЛЕНИЕ либо частичные платежи, либо дыры в данных

   Различие реальное и полезное, но подписи бакетов § 3 читать именно так.
   Величину просрочки даёт только слой 1 (взносы: план против факта).
   Порядок прогона: сначала D15_C_dueday.sql (П3), потом этот скрипт.
...
CROSS APPLY (VALUES (
        CASE WHEN c.dpd_sd IS NULL THEN '7 — не считается'
             WHEN c.dpd_sd  = 0    THEN '1 — перекат, картина не меняется'
             WHEN c.dpd_sd <= 1    THEN '2 — перекат, разброс до 1 дня'
             WHEN c.dpd_sd <= 2    THEN '3 — 1-2 дня'
             WHEN c.dpd_sd <= 4    THEN '4 — 2-4 дня'
             WHEN c.dpd_sd <= 8    THEN '5 — 4-8 дней'
             ELSE                       '6 — накопление либо дыры, разброс > 8'
        END)) AS b(sd_bucket)
WHERE c.stage_asof      = '3'
  AND c.has_asof_snap   = 1
  AND c.months_with_dpd = @WindowMonths
...
/* -----------------------------------------------------------------------------
   ...
   §3  Доля пула в бакетах 1 и 2 — это ПЕРЕКАТ: заёмщик платит каждый месяц,
       но с опозданием. Верхняя оценка маршрута C, а не его размер: величина
       опоздания из снимков не следует (METHOD.md, П3). Колонка
       late_every_month отделяет регулярный перекат от эпизодического.
