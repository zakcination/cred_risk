/* =============================================================================
   ...
   ВНИМАНИЕ: в таблице есть колонка IIN — персональные данные. SELECT *
   по ней запрещён. Любая выборка перечисляет колонки явно и IIN не включает.
...
-- 0c. Если строк на счёт больше одной — что их различает.
--     ИСПРАВЛЕНО 27.08.2026: здесь стоял SELECT *, и он вытащил колонку IIN.
--     В таблице лежат ПЕРСОНАЛЬНЫЕ ДАННЫЕ (IIN, ORDINAL_POSITION 2), а правило
--     контура запрещает их выводить. SELECT * по этой таблице не делать НИКОГДА.
--     Ниже — только различающие поля, без идентификаторов заёмщика.
SELECT TOP 50
      account_number, [status], subproduct, TARIFF, DURATION
    , FIRST_PAYMENT_DATE, activation_date
